...
type Filter = "all" | CenterId;

const ABNA_RASHID_LABEL = "Abna Rashid Bldg.";

const filterLabels: Record<CenterId, { short: string; city: string }> = {
  "smart-creation": { short: "Smart Creation BC", city: "Tecom · Dubai" },
  "smart-place": { short: "Smart Place BC", city: "Al Barsha · Dubai" },
  "smart-view": { short: "Smart View BC", city: "Bur Dubai" },
  "future-space": { short: "Future Space BC", city: "Al Muraqabat · Dubai" },
  "abna-rashid": { short: ABNA_RASHID_LABEL, city: "Deira · Dubai" },
};
...
export function Offices() {
  ...
  const counts = useMemo(() => {
    ...
    for (const o of officeListings) c[o.centerId]++;
    return c;
  }, []);
  ...
  const visible = useMemo(
    () =>
      filter === "all"
        ? officeListings
        : officeListings.filter((o) => o.centerId === filter),
    [filter]
  );
    ...
    <section id="offices" className="relative py-24 md:py-36 bg-paper-deep">
      <div className="container-edit">
        ...
        >
          ...
          />
          {groupCenters.map((c) => {
            const id = c.id as CenterId;
            const meta = filterLabels[id];
  ...
  return (
    <article className="group flex flex-col h-full rounded-3xl border border-ink/10 bg-paper overflow-hidden transition-all hover:border-ink/25 hover:shadow-[0_22px_60px_-30px_rgba(13,16,19,0.28)]">
      ...
      >
        ...
        <div
          aria-hidden
          className="absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-ink/45 to-transparent"
        />
      ...
      <div className="flex flex-col flex-1 p-6 md:p-7">
        ...
        <div className="mt-auto pt-6 border-t border-ink/10 flex items-end justify-between gap-3">
          ...
          <Link
            href={`/business-centers/${office.id}`}
